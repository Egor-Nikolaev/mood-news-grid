...
function migrate(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS rewrites (
      id             INTEGER PRIMARY KEY AUTOINCREMENT,
      text_key       TEXT NOT NULL,        -- хэш оригинального текста (см. lib/facts.js textKey)
      ...
      UNIQUE(text_key, mood)               -- кэш адресуется содержимым, а не id статьи
    );
  `);
}
...
export function getArticleByGuid(guid) {
  return getDb().prepare(`SELECT * FROM articles WHERE guid = ?`).get(guid);
}
...
// --- rewrites (кэш, адресуется хэшом текста) ---
export function getRewrite(textKey, mood) {
  return getDb()
    .prepare(`SELECT * FROM rewrites WHERE text_key = ? AND mood = ?`)
    .get(textKey, mood);
}
...
export function saveRewrite(r) {
  const db = getDb();
  db.prepare(`
    INSERT INTO rewrites (text_key, mood, text, method, verified, violations_json, created_at)
    VALUES (@text_key, @mood, @text, @method, @verified, @violations_json, @created_at)
    ON CONFLICT(text_key, mood) DO UPDATE SET
      text = excluded.text,
