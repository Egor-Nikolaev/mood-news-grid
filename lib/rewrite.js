...
// Провайдер-агностика: любой OpenAI-совместимый endpoint (Groq, OpenRouter, ...).
// Приоритет — новые LLM_* переменные; старые OPENROUTER_* остаются как фолбэк.
...
async function callLLM(messages) {
  const model =
    process.env.LLM_MODEL || process.env.OPENROUTER_MODEL || "llama-3.3-70b-versatile";
  // Даже на платных/щедрых провайдерах бывает 429/503 (общий лимит) —
