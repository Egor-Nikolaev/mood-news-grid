...
function systemPrompt(mood) {
  return [
    "Ты переписываешь новость в заданном эмоциональном ТОНЕ. Отвечай на русском языке.",
    "ЖЁСТКИЕ ПРАВИЛА — это задача на сохранение фактов, а не творческая:",
    "- НЕЛЬЗЯ добавлять, убирать или менять факты: имена, места, организации, числа, деньги, проценты, даты и цитаты должны остаться дословно.",
    "- НЕЛЬЗЯ выдумывать новые факты или последствия.",
    "- Меняй только формулировки, подачу и эмоциональную окраску.",
    "- Сохраняй примерно ту же длину. Выведи ТОЛЬКО переписанный текст, без вступлений.",
    `ТОН: ${MOODS[mood].llm}`,
  ].join("\n");
}
  ...
  try {
    ...
    text = await callLLM([
      ...
      { role: "user", content: originalText },
      {
        role: "user",
        content:
          "В твоём варианте потеряны или искажены эти факты. Перепиши заново в том же тоне, " +
          "вернув каждый из них ДОСЛОВНО: " + missing,
      },
    ]);
